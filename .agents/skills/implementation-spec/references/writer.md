...
6. For any new top-level spec file, allocate one new document ID by locking `<artifact-dir>/.lock`. Create the lock file if it does not exist. Treat the file contents as the last allocated implementation-spec ID.
...
8. While holding the lock, read the last allocated ID, increment it to the next ID allowed by `name_pattern`, write the new last ID back to `.lock`, and release the lock. If the file is empty, allocate the first valid ID for that pattern. Render the top-level filename from `name_pattern`, then set frontmatter `id` on that top-level file to the rendered filename stem without the `.md` extension.
